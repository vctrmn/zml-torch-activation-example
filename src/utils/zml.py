...
class ActivationCollector:
    ...
    def __call__(self, *args, **kwargs):
        ...
        """
        self.count = 0
        hook = torch.nn.modules.module.register_module_forward_hook(self.log_activation_hook)
...
def named_modules(model):
    if hasattr(model, "named_modules"):
        ...
        root_modules = [(k, v) for k, v in model.__dict__.items() if isinstance(v, torch.nn.Module)]
